...
with tab_log:
    ...
    if not df.empty:
        ...
        # 에러 방지용 체크
        top_mood = df['기분'].mode()[0] if not df['기분'].empty else "알 수 없음"
        top_obj = df['기물종류'].mode()[0] if not df['기물종류'].empty else "알 수 없음"
        ...
        <div class="dana-card">
            <p style='line-height:1.8;'>
                ...
                주로 흙을 만질 때 <span class="highlight">'{top_mood}'</span> 마음이었고,<br>
                가장 많이 만든 건 <span class="highlight">'{top_obj}'</span>이에요!
            </p>
        </div>
        """, unsafe_allow_html=True)
            ...
            with st.expander(f"🏺 {row['날짜']} | {row['작품명']} ({row['단계']})"):
                ...
                with st.popover("✏️ 모든 정보 수정"):
                    with st.form(f"full_edit_{idx}"):
                        e_mood = st.radio("기분", list(MOOD_DICT.keys()), index=list(MOOD_DICT.keys()).index(row['기분']) if row['기분'] in MOOD_DICT else 0, horizontal=True, format_func=lambda x: MOOD_DICT[x])
                        ...
                        e_imgs = st.file_uploader("사진 교체(선택 시 기존 사진 삭제됨)", type=["jpg", "png", "jpeg"], accept_multiple_files=True, key=f"f_edit_img_{idx}")
                        
                        if st.form_submit_button("수정 저장"):
                            # df.at을 이용해 원본 데이터프레임의 올바른 행 수정
                            df.loc[idx, '기분'] = e_mood
                            df.loc[idx, '날짜'] = e_date
                            df.loc[idx, '작품명'] = e_title
                            df.loc[idx, '작업유형'] = e_type
                            df.loc[idx, '흙'] = e_clay
                            df.loc[idx, '기물종류'] = e_obj
                            df.loc[idx, '단계'] = e_step
                            df.loc[idx, '내용'] = e_note
                            
                            if e_imgs:
                                new_img_list = [process_img(e_imgs[i]) if i < len(e_imgs) else "" for i in range(3)]
                                df.loc[idx, '사진1'] = new_img_list[0]
                                df.loc[idx, '사진2'] = new_img_list[1]
                                df.loc[idx, '사진3'] = new_img_list[2]
                                
                            save_data(df)
                            st.success("수정되었습니다!")
                            st.rerun()

                if st.button("🗑️ 삭제", key=f"dl_{idx}"): 
...
with tab_mood:
    ...
    if not df.empty:
        ...
        if mode == "📅 월별":
            ...
            v_m = int(sel_m[6:8])
            f_df = df[pd.to_datetime(df['날짜']).dt.month == v_m]
            d_n = f"{v_m}월"
        else:
            p_v = st.selectbox("작품 선택", sorted(df['작품명'].unique()), key="mood_p_final")
            f_df = df[df['작품명'] == p_v]
            d_n = p_v
            if not f_df.empty and pd.notna(f_df.iloc[-1]['사진1']) and f_df.iloc[-1]['사진1'] != "":
        ...
        if not f_df.empty:
            ...
            for i, (name, emoji) in enumerate(items):
                ...
                pct = int(cnt/total_m*100) if total_m > 0 else 0
                if i == 4: h_mood += '</tr><tr>'
...
with tab_rec:
    ...
    with st.form("rec_form", clear_on_submit=True):
        ...
        c3, c4 = st.columns(2)
        r_type = c3.selectbox("작업 유형", TYPE_LIST)
        r_clay = c4.selectbox("흙 종류", CLAY_LIST)
        r_obj = st.selectbox("기물 종류", OBJ_LIST)
        r_step = st.select_slider("단계", options=STEP_LIST)
