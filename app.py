...
if 'selected_proj_name' not in st.session_state: st.session_state.selected_proj_name = None
...
with tab_rec:
    ...
    with st.form("rec_f", clear_on_submit=True):
        ...
        c1, c2 = st.columns(2); r_date = c1.date_input("날짜", datetime.now().date()); r_title = c2.text_input("작품명")
...
# --- [TAB 4: 🏺 작품 모아보기 - 9칸 그리드 고정 및 상세보기 최적화] ---
with tab_proj:
    st.markdown("<div class='title-text'>작품 모아보기</div>", unsafe_allow_html=True)
    p_filter = st.radio("필터", ["전체", "작업중", "완성"], horizontal=True, key="p_f_final_insta")
    ...
    for i in range(max(9, len(display_list))):
        if i < len(display_list):
            ...
            grid_html += f'<div class="insta-box">{"<img src=\'"+src+"\'>" if src else "<span style=\'color:#ccc; font-size:0.6em;\'>No Pic</span>"}</div>'
        else:
            grid_html += '<div class="insta-box insta-empty"></div>'
    grid_html += '</div>'
    st.markdown(grid_html, unsafe_allow_html=True)

    # 이미지 아래에 '보기' 버튼들을 별도로 배치하여 클릭 감지
    btn_cols = st.columns(3)
    for i in range(len(display_list)):
        with btn_cols[i % 3]:
            st.markdown('<div class="grid-view-btn">', unsafe_allow_html=True)
            if st.button(f"🔍", key=f"view_proj_{i}"):
                st.session_state.selected_proj_name = display_list[i]
            st.markdown('</div>', unsafe_allow_html=True)

    # 작품 클릭 시 상세 히스토리 (역시 그리드 형태로 보여줌)
    if st.session_state.selected_proj_name:
        st.divider()
        st.markdown(f"**🏺 {st.session_state.selected_proj_name} 작업 히스토리**")
        proj_df = df[df['작품명'] == st.session_state.selected_proj_name].sort_values(by='날짜')
        
        # 상세 페이지도 이미지가 너무 커지지 않게 그리드로 배치
        for _, r in proj_df.iterrows():
            st.markdown(f"<small>{r['날짜']} | {r['단계']}</small>", unsafe_allow_html=True)
            h_cols = st.columns(3)
            for i, cn in enumerate(['사진1', '사진2', '사진3']):
                if pd.notna(r[cn]) and r[cn] != "":
                    h_cols[i].image(base64.b64decode(r[cn]), use_container_width=True)
...
with tab_mood:
    ...
    if mood_mode == "📅 월별":
        sel_m = st.selectbox("월 선택", month_opts, index=st.session_state.sel_m_idx, key="mood_m")
