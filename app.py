...
with tab_proj:
    ...
    p_filter = st.radio("필터", ["전체", "작업중", "완성"], horizontal=True, key="pf_insta")
    ...
    if st.session_state.proj_detail:
        st.divider()
        st.markdown(f"**🏺 {st.session_state.proj_detail} 상세보기**")
...
with tab_mood:
    ...
    if not f_df.empty:
        counts = f_df['기분'].value_counts(); total = len(f_df)
        st.markdown(f"<div class='summary-box'>✨ {d_n}에는 주로 <b>'{counts.idxmax()}'</b>였네요!</div>", unsafe_allow_html=True)
        ...
        for i, (name, emoji) in enumerate(items):
            cnt = counts.get(name, 0); pct = int(cnt/total*100)
            if i == 4: h_mood += '</tr><tr>'
...
# --- [TAB 6: 📊 기록 요약 - 요약 멘트 및 3종 그래프 완벽 복원] ---
with tab_log:
    st.markdown("<div class='title-text'>DANA의 기록 요약</div>", unsafe_allow_html=True)
    if not df.empty:
        done = df[df['단계'] == "완성"]['작품명'].nunique(); ing = df['작품명'].nunique() - done
        # 멘트 복원
        st.markdown(f"""
        <div class="dana-card">
            <p style='line-height:1.8;'>
                지금까지 Dana님은...<br>
                총 <span class="highlight">{done}개</span>의 작품을 완성하고,<br>
                지금은 <span class="highlight">{ing}개</span>의 아이들을 빚고 있어요. ✨<br>
