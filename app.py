...
def save_data(df):
    temp = df.copy(); temp['날짜'] = temp['날짜'].astype(str)
...
with tab_log:
    ...
    if not df.empty:
        done_p = df[df['단계'] == "완성"]['작품명'].nunique(); total_p = df['작품명'].nunique(); ing_p = total_p - done_p
        top_m = df['기분'].mode()[0] if not df['기분'].empty else "-"
        top_o = df['기물종류'].mode()[0] if not df['기물종류'].empty else "-"
        st.markdown(f"""<div class="dana-card"><p style='line-height:1.8;'>지금까지 Dana님은...<br>총 <span class="highlight">{done_p}개</span>의 작품을 완성하고,<br>지금은 <span class="highlight">{ing_p}개</span>의 아이들을 빚고 있어요. ✨<br>주로 흙을 만질 때 <span class="highlight">'{top_m}'</span> 마음이었고,<br>가장 많이 만든 건 <span class="highlight">'{top_o}'</span>이에요!</p></div>""", unsafe_allow_html=True)
        ...
        for day in week:
            if day == 0: h_cal += '<td></td>'
                ...
                if not logs.empty:
                    ...
                    if str(last_log['사진1']) != "":
                        thumb = f'<img src="data:image/jpeg;base64,{last_log["사진1"]}" style="width:30px;height:30px;object-fit:cover;border-radius:4px;">'
...
with tab_list:
    ...
    if not df.empty:
        sorted_list_df = df.sort_values(by='날짜', ascending=False)
        for idx, row in sorted_list_df.iterrows():
            with st.expander(f"🏺 {row['날짜']} | {row['작품명']} ({row['단계']})"):
                ...
                for i, c in enumerate(['사진1', '사진2', '사진3']):
                    if str(row[c]) != "": icols[i].image(base64.b64decode(row[c]), use_container_width=True)
                with st.popover("✏️ 모든 정보 수정"):
                    with st.form(f"f_edit_{idx}"):
                        e_m = st.radio("기분", list(MOOD_DICT.keys()), index=list(MOOD_DICT.keys()).index(row['기분']) if row['기분'] in MOOD_DICT else 0, horizontal=True, format_func=lambda x: MOOD_DICT[x])
                        e_d = st.date_input("날짜", row['날짜']); e_t = st.text_input("작품명", row['작품명'])
                        e_ty = st.selectbox("유형", TYPE_LIST, index=TYPE_LIST.index(row['작업유형']) if row['작업유형'] in TYPE_LIST else 0)
                        e_cl = st.selectbox("흙", CLAY_LIST, index=CLAY_LIST.index(row['흙']) if row['흙'] in CLAY_LIST else 0)
                        e_ob = st.selectbox("기물", OBJ_LIST, index=OBJ_LIST.index(row['기물종류']) if row['기물종류'] in OBJ_LIST else 0)
                        e_st = st.select_slider("단계", options=STEP_LIST, value=row['단계'])
                        e_no = st.text_area("메모", value=str(row['내용']))
                        e_img = st.file_uploader("사진 교체", type=["jpg", "png", "jpeg"], accept_multiple_files=True, key=f"fe_{idx}")
                        if st.form_submit_button("수정 저장"):
                            df.loc[idx, ['기분','날짜','작품명','작업유형','흙','기물종류','단계','내용']] = [str(e_m), e_d, str(e_t), str(e_ty), str(e_cl), str(e_ob), str(e_st), str(e_no)]
                            if e_img:
                                img_l = [process_img(e_img[i]) if i < len(e_img) else "" for i in range(3)]
...
with tab_proj:
    ...
    if not df.empty:
        ...
        display_list = [t for t in u_titles if (p_filter=="전체") or (p_filter=="작업중" and "완성" not in df[df['작품명']==t]['단계'].values.tolist()) or (p_filter=="완성" and "완성" in df[df['작품명']==t]['단계'].values.tolist())]
        grid_html = '<div class="insta-container">'
        for i in range(max(9, len(display_list))):
            if i < len(display_list):
                t = display_list[i]; p_logs = df[df['작품명'] == t]
                # 사진이 있는 행만 정확히 필터링
                has_photo_logs = p_logs[p_logs['사진1'].apply(lambda x: str(x) != "")]
                rep = has_photo_logs.iloc[-1] if not has_photo_logs.empty else None
        ...
        if st.session_state.proj_detail:
            ...
            for _, r in p_hist.iterrows():
                ...
                for i, cn in enumerate(['사진1','사진2','사진3']):
                    if str(r[cn]) != "": ic[i].image(base64.b64decode(r[cn]), use_container_width=True)
...
with tab_mood:
    ...
    if not df.empty:
        ...
        if mode == "📅 월별":
            sel_m = st.selectbox("분석 월 선택", month_opts, index=st.session_state.sel_m_idx, key="md_m")
            v_m = int(sel_m[6:8]); f_df = df[pd.to_datetime(df['날짜']).dt.month == v_m]; d_n = f"{v_m}월"
        else:
            p_v = st.selectbox("작품 선택", sorted(df['작품명'].unique()), key="md_p")
            f_df = df[df['작품명'] == p_v]; d_n = p_v
            # [이미지 복구 부분] 작품 선택 시 대표 이미지 표시
            if not f_df.empty:
                has_photo = f_df[f_df['사진1'].apply(lambda x: str(x) != "")]
                if not has_photo.empty:
                    rep_pic = has_photo.iloc[-1]['사진1']
                    st.markdown(f'<div style="width:100px; margin:0 auto 10px; aspect-ratio:1/1; overflow:hidden; border-radius:8px; border:1px solid #EEE;"><img src="data:image/jpeg;base64,{rep_pic}" style="width:100%; height:100%; object-fit:cover;"></div>', unsafe_allow_html=True)
        if not f_df.empty:
...
with tab_rec:
    ...
    with st.form("rec_form", clear_on_submit=True):
        ...
        c1, c2 = st.columns(2)
        r_d = c1.date_input("날짜", datetime.now().date()); r_t = c2.text_input("작품명")
        c3, c4 = st.columns(2)
        r_ty = c3.selectbox("유형", TYPE_LIST); r_cl = c4.selectbox("흙 종류", CLAY_LIST)
        r_ob = st.selectbox("기물 종류", OBJ_LIST); r_st = st.select_slider("단계", options=STEP_LIST)
        r_ims = st.file_uploader("사진(최대 3장)", type=["jpg", "png", "jpeg"], accept_multiple_files=True)
        r_no = st.text_area("메모")
        if st.form_submit_button("기록 저장하기"):
            if r_t:
                img_l = [process_img(r_ims[i]) if r_ims and i < len(r_ims) else "" for i in range(3)]
                new_row = pd.DataFrame([[r_d, r_t, r_cl, r_st, r_no, img_l[0], img_l[1], img_l[2], mood, r_ty, r_ob]], columns=df.columns)
