...
st.markdown(f"""
    <style>
    /* 손글씨 웹폰트 불러오기 (교보손글씨 2019) */
    @import url('https://cdn.jsdelivr.net/gh/projectnoonnu/noonfonts_20-04@2.1/KyoboHandwriting2019.woff');
    
    html, body, [data-testid="stAppViewContainer"] {{
        font-family: 'KyoboHandwriting2019', sans-serif !important;
        background-color: #FDFBF9;
        color: #5D574F;
    }}
    
    /* 입력을 위한 폼 요소들은 가독성을 위해 기본 폰트 유지하거나 크기 키움 */
    ...
    .stTabs [data-baseweb="tab"] {{ font-size: 1.5em !important; padding: 10px 0px; }}
    ...
    .gallery-title {{ font-weight: bold; font-size: 1.1em; color: #444; display: flex; align-items: center; gap: 5px; }}
    .complete-badge {{ background-color: #E2F0CB; color: #5D8A66; font-size: 0.7em; padding: 2px 6px; border-radius: 5px; font-weight: bold; }}

    /* 캘린더 */
    .cal-table {{ width: 100%; border-collapse: collapse; table-layout: fixed; }}
    .cal-table th {{ text-align: center; font-size: 0.8em; color: {MAIN_COLOR}; padding: 5px 0; }}
    .cal-table td {{ border: 1px solid #F0F0F0; background: white; height: 60px; vertical-align: top; padding: 4px 1px; text-align: center; }}
    .cal-date-num {{ font-size: 0.7em; color: #CCC; display: block; margin-bottom: 2px; }}
    .cal-mood-sticker {{ font-size: 1.3em; display: block; }}
    ...
    .summary-box {{ background: #F9F5F2; padding: 15px; border-radius: 15px; border-left: 5px solid {MAIN_COLOR}; margin-top: 10px; font-size: 1.1em; }}
    .highlight {{ color: #D4A373; font-weight: bold; font-size: 1.2em; }}
    .title-text {{ font-size: 1.8em; font-weight: bold; color: #5D574F; margin-bottom: 15px; }}
    .stButton>button {{ width: 100%; border-radius: 15px; height: 3.5em; background-color: {MAIN_COLOR}; color: white; font-weight: bold; border: none; font-size: 1.1em !important; }}
    </style>
            ...
            with p_cols[idx % 2]:
                ...
                <div class="gallery-card">
                    <div class="gallery-img-container">
                        {'<img src="' + img_src + '">' if rep_row is not None else '<div style="padding:40% 10%; color:#ccc; font-size:0.8em;">No Photo</div>'}
                    </div>
                    <div class="gallery-info">
                        <div class="gallery-title">🏺 {t} {'<span class="complete-badge">완성</span>' if is_done else ''}</div>
                    </div>
                </div>
                """, unsafe_allow_html=True)
...
with tab_mood:
    ...
    c1, c2 = st.columns(2)
    with c1: m_month = st.selectbox("월 선택", list(range(1, 13)), index=datetime.now().month-1, key="sm")
    with c2: m_proj = st.selectbox("작품별", ["전체보기"] + list(df['작품명'].unique()), key="sp")
    ...
    if not m_df.empty:
        ...
        for i, (m, emoji) in enumerate(MOOD_DICT.items()):
            ...
            with m_grid[i % 4]: st.markdown(f"<div style='text-align:center;'>{emoji}<br><small style='color:#999;'>{m}</small><br><b>{cnt}</b></div>", unsafe_allow_html=True)
    else: st.info("기록이 없습니다.")
...
with tab_log:
    ...
    if not df.empty:
        ...
        st.markdown(f"""
        <div class="dana-card">
            <p style='line-height:2.0; font-size:1.15em;'>
                지금까지 Dana님은...<br>
                총 <span class="highlight">{done_p}개</span>를 완성하고,<br>
                지금은 <span class="highlight">{ing_p}개</span>를 빚고 있어요. 🕊️<br>
                주로 <span class="highlight">'{top_m}' {MOOD_DICT.get(top_m, '')}</span> 마음이었고,<br>
                주력 기물은 <span class="highlight">'{top_o}'</span>이에요!
            </p>
        </div>
        """, unsafe_allow_html=True)
        def draw_donut(labels, values):
            ...
            fig.update_layout(showlegend=True, margin=dict(t=10, b=10, l=10, r=10), height=300, legend=dict(orientation="h", y=-0.1))
            st.plotly_chart(fig, use_container_width=True)
        draw_donut(df['기분'].value_counts().index, df['기분'].value_counts().values)
        draw_donut(df['기물종류'].value_counts().index, df['기물종류'].value_counts().values)
