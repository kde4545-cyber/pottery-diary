            ...
            with st.expander(f"🏺 {row['날짜']} | {row['작품명']} ({row['단계']})"):
                ...
                with st.popover("✏️ 정보 수정"):
                    with st.form(f"full_edit_{idx}"):
                        ...
                        e_imgs = st.file_uploader("사진 교체 (3장 선택 시 기존 사진 삭제됨)", type=["jpg", "png", "jpeg"], accept_multiple_files=True, key=f"f_edit_img_{idx}")
...
with tab_mood:
    ...
    if mode == "📅 월별":
        ...
        p_v = st.selectbox("작품 선택", sorted(df['작품명'].unique()) if not df.empty else [], key="mood_p_final")
        f_df = df[df['작품명'] == p_v] if p_v else pd.DataFrame(); d_n = p_v
