...
st.markdown(f"""
    ...
    /* [작품 모아보기] 인스타그램형 3x3 그리드 고정 (제목 위, 이미지 아래) */
    .insta-container {{
        display: grid;
...
# --- [TAB 4: 🏺 작품 모아보기 - 3x3 인스타그램 그리드] ---
with tab_proj:
    st.markdown("<div class='title-text'>작품 모아보기</div>", unsafe_allow_html=True)
    p_filter = st.radio("필터", ["전체", "작업중", "완성"], horizontal=True, key="p_f_final")
        ...
        if i < len(display_list):
            ...
            grid_html += f'''
                <div class="insta-item">
                    ...
                    <div class="insta-box">{"<img src='"+src+"'>" if src else "<span style='color:#ccc; font-size:0.6em;'>No Pic</span>"}</div>
                </div>'''
        else:
            grid_html += '<div class="insta-item"><div class="insta-title"></div><div class="insta-box insta-empty"></div></div>'
    ...
    if not df.empty:
        ...
        <div class="dana-card">
            <p style='line-height:1.8;'>
                ...
                주로 흙을 만질 때 <span class="highlight">'{top_m}'</span> 마음이었고,<br>
                가장 많이 만든 건 <span class="highlight">'{top_o}'</span>이에요!
            </p>
        </div>
        """, unsafe_allow_html=True)
