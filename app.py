...
st.markdown(f"""
    ...
    html, body, [data-testid="stAppViewContainer"], .stMarkdown, p, div, span, label, input, select, textarea, button {{
        ...
        font-size: 1.25em !important; /* 손글씨는 크게 봐야 예쁩니다 */
        color: #5D574F;
    }}
    ...
    .cal-table td {{ border: 1px solid #F0F0F0; background: white; height: 75px; vertical-align: top; padding: 4px 1px; text-align: center; overflow: hidden; }}
    ...
    .cal-mood-sticker {{ font-size: 1.1em; }}
...
with tab_cal:
    ...
    if work_days > 0:
        ...
        st.write(f"이번 달은 총 **{work_days}일** 작업했어요. 가장 많이 느낀 기분은 **'{top_m}' {MOOD_DICT[top_m]}**이네요!")
                    ...
                    for r_idx, row in p_logs.iterrows():
                        ...
                        for img_col in ['사진1', '사진2', '사진3']:
                            if pd.notna(row[img_col]) and row[img_col] != "":
                                st.image(base64.decodebytes(row[img_col].encode()), use_container_width=True)
