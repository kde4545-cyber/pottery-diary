...
st.markdown(f"""
    ...
    /* 모바일 고정 캘린더 디자인 (Table 방식) */
    .cal-table {{
        width: 100%;
        border-collapse: collapse;
        table-layout: fixed; /* 모든 칸 너비 동일 고정 */
    }}
    .cal-table th {{
        text-align: center;
        font-size: 0.7em;
    ...
    for week in cal_obj:
        ...
        for day in week:
            if day == 0:
                ...
                today_cls = 'is-today' if curr_date == today_date else ''
                html += f'<td class="{today_cls}"><span class="cal-date-num">{day}</span><span class="cal-mood-sticker">{sticker}</span></td>'
        html += '</tr>'
    html += '</tbody></table>'
...
with tab_proj:
    ...
    if not df.empty:
        ...
        for idx, t in enumerate(display_titles):
            ...
            # 대표 이미지: 완성 단계 우선, 없으면 가장 최신 이미지
            finished_img = p_logs[(p_logs['단계'] == "완성") & (p_logs['사진'] != "")]
            rep_row = finished_img.iloc[-1] if not finished_img.empty else (p_logs[p_logs['사진'] != ""].iloc[-1] if not p_logs[p_logs['사진'] != ""].empty else None)
            ...
            with proj_cols[idx % 2]:
                with st.expander(f"{'✅' if '완성' in p_logs['단계'].values else '🏺'} {t}"):
                    ...
                    for r_idx, row in p_logs.iterrows():
                        ...
                        if row['사진']: st.image(base64.b64decode(row['사진']), use_container_width=True)
...
with tab_mood:
    ...
    m_month = st.selectbox("월 선택", list(range(1, 13)), index=datetime.now().month-1, key="sel_m_mood")
    m_proj = st.selectbox("작품별 선택", ["전체보기"] + list(df['작품명'].unique()))
...
with tab_log:
    ...
    if not df.empty:
        ...
        st.markdown(f"""<div class="dana-card"><p style='line-height:1.8; font-size:1.05em;'>지금까지 Dana님은...<br>총 <span class="highlight">{done_p}개</span>를 완성하고,<br>지금은 <span class="highlight">{ing_p}개</span>를 빚고 있어요. 🕊️<br>주로 <span class="highlight">'{top_m}' {MOOD_DICT.get(top_m, '')}</span> 마음이었고,<br>가장 많이 만든 건 <span class="highlight">'{top_o}'</span>이에요!</p></div>""", unsafe_allow_html=True)
        ...
        def draw_pastel_donut(labels, values):
            ...
            fig.update_layout(showlegend=True, margin=dict(t=10, b=10, l=10, r=10), height=300, legend=dict(orientation="h", y=-0.2))
